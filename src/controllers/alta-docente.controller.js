...
export function altaDocentes(req, res) {
	...
	try {
		if (rol_creador >= 1) {
			//TODO: habilitar permisos cuando funcionen
			var salt = bcrypt.genSaltSync(10);
			var hash = bcrypt.hashSync(password, salt);
			db.all(
				insertDocentes,
				[nombre, tipo_dni, nro_dni, id_rol, hash, apellido, activo, fecha_nac, nro_legajo],
				(err, rows) => {
					if (err) {
						console.log(err.message);
						return res.json({ mensaje: err.message }).status(500);
			...
			db.all("SELECT MAX(id_docente) as id FROM docentes", (err, row) => {
				...
				db.all(
					insertDetalleDocentes,
					[id_docente, direccion, localidad, car_telefono, telefono, car_tel_extra, telefono_extra, email],
					(err, rows) => {
						if (err) {
							console.log(err.message);
							return res.json({ mensaje: err.message }).status(500);
