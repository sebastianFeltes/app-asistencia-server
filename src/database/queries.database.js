...
WHERE CUR.id_curso = ? ORDER BY ALU.apellido;`;
...
export const insertCurso = `INSERT INTO cursos (nombre ,id_docente, horario_inicio, horario_final,activo, fecha_inicio,fecha_final, horas_catedra,cantidad_clases,cantidad_inasistencias) VALUES(?,?,?,?,?,?,?,?,?,?)`;
...
export const insertDetalleAlumno = `INSERT INTO detalle_alumnos (id_alumno,direccion, localidad,car_telefono, telefono, car_tel_extra, telefono_extra, email, lugar_nacimiento, nacionalidad, fotoc_dni, fotoc_analitico, planilla_ins ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`;
...
export const updateDocente = `UPDATE docentes SET nro_legajo=?, nombre =?, tipo_dni =?, nro_dni=?, id_rol =?, apellido =?, activo =?, fecha_nac =? WHERE id_docente = ?`;
...
export const updateCursos = `UPDATE cursos SET nombre=?, id_docente=?, horario_inicio=?, horario_final=?, activo=?, fecha_inicio=?, fecha_final=?, horas_catedra=?, cantidad_clases = ?, cantidad_inasistencias = ? WHERE id_curso=?`;
